...
enum TransferDatabaseBuilder {
    ...
    /// One row per page file, describing that page's position within a
    /// paginated `.sqlite` transfer walk. Written to a second table
    /// (`transfer_manifest`) in the same file as `items` — see `build(rows:
    /// manifest:at:)` — so the manifest can never arrive out of sync with the
    /// row data it describes (unlike an HTTP header, which could be stripped
    /// or altered in transit independently of the body).
    struct ManifestRow {
        let walkID: String              // UUID, generated by the server on page 1, echoed by caller on page 2+
        let pageNumber: Int              // 1-indexed
        let hasMore: Bool
        let pageRowCount: Int            // rows in *this* file's items table
        let expectedTotalRowCount: Int   // fixed at walk start, same for every page of the walk
    }
    ...
    /// stamped. `fileURL` must not already exist. `manifest` describes this
    /// page's position within a paginated walk (see `ManifestRow`) and is
    /// written to a second table, `transfer_manifest`, alongside `items`.
    static func build(rows: [Row], manifest: ManifestRow, at fileURL: URL) throws {
        let db: Connection
        do {
        ...
        CREATE TABLE transfer_manifest (
            ...
            expected_total_row_count  INTEGER NOT NULL
        );
        """)
        ...
        try db.transaction {
            try db.run(
                ...
                [manifest.walkID, manifest.pageNumber, manifest.hasMore ? 1 : 0,
                 manifest.pageRowCount, manifest.expectedTotalRowCount]
            )
            for row in rows {
